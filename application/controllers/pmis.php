...
class pmis extends CI_Controller
{
	...
	public function ajax_list()
	{
		...
		foreach ($list as $item) {
			...
			$row[] = $no;
			$row[] = $item->activity;
			$row[] = date('d-M-Y H:i', strtotime($item->waktu));
	...
	public function aksi_project_add()
	{
		...
		if ($this->session->userdata('status') == 2 || $this->session->userdata('status') == 3) {
			...
			$data = array(
				'id_upp' => $id_upp, 'kode_project' => $kode_project, 'nama_project' => $nama_project, 'program_project' => $program_project, 'fase_project' => $fase_project,
	...
	public function aksi_general_information_add()
	{
		...
		if ($this->session->userdata('status') == 2 || $this->session->userdata('status') == 3) {
			...
			$data = array(
				'id_project' => $id_project, 'lokasi_project' => $lokasi_project,
	...
	public function aksi_milstone_add()
	{
		...
		if ($this->session->userdata('status') == 2 || $this->session->userdata('status') == 3) {
			...
			$data = array(
				'id_project' => $id_project, 'nama_milstone' => $nama_milstone,
	...
	public function aksi_kontrak_utama_add()
	{
		...
		if ($this->session->userdata('status') == 2 || $this->session->userdata('status') == 3) {
			...
			if ($id_projetc = 61) {
				...
				$data = array(
					'id_project' => $id_project, 'nama_kontrak_utama' => $nama_kontrak_utama, 'pelaksana' => $pelaksana, 'nomor_kontrak' => $nomor_kontrak,
	...
	public function aksi_kontrak_utama_edit()
	{
		...
		if ($this->session->userdata('status') == 2 || $this->session->userdata('status') == 3) {
			...
			$data = array(
				'id_project' => $id_project, 'nama_kontrak_utama' => $nama_kontrak_utama, 'pelaksana' => $pelaksana,
				'jenis_kontrak' => $jenis_kontrak, 'tanggal_kontrak' => $tanggal_kontrak,
				'effective_date' => $effective_date, 'nilai_kontrak' => $nilai_kontrak, 'tanggal_berakhir1' => $tanggal_berakhir1, 'tanggal_berakhir2' => $tanggal_berakhir2,
				'tanggal_berakhir3' => $tanggal_berakhir3, 'tanggal_berakhir4' => $tanggal_berakhir4, 'tanggal_berakhir5' => $tanggal_berakhir5,
				'tanggal_berakhir6' => $tanggal_berakhir6, 'tanggal_berakhir7' => $tanggal_berakhir7,
	...
	public function aksi_kontrak_supervisi_add()
	{
		...
		if ($this->session->userdata('status') == 2 || $this->session->userdata('status') == 3) {
			...
			//Finish Log Activity Add
			$data = array(
				'id_project' => $id_project, 'nama_kontrak_supervisi' => $nama_kontrak_supervisi, 'pelaksana' => $pelaksana,
	...
	public function ajax_kontrak_engineering()
	{
		$id_project = $this->input->get('id_project');
		$dapat_status = $this->pmis_model->dapat_status($this->session->userdata('status'));
		$jumlah_mesin = $this->pmis_model->cek_mesin($id_project)['jumlah_mesin'];
		...
		foreach ($list as $item) {
			...
			$row = array();
			$row[] = $no;
			$row[] = $item['nama_kontrak_engineering'];
			$row[] = $item['pelaksana'];
			$row[] = $item['nomor'];
			$row[] = $item['tanggal_ttd'];
			$row[] = $item['jenis_kontrak'];
			$row[] = $item['nilai_kontrak'];
			$row[] = $item['tanggal_berakhir'];
			$row[] = $item['status'];
			...
			if ($dapat_status['status'] == 4) {
				...
				$row[] = '<td>
					<input type="hidden" name="id_project" value="' . $item['id_project'] . '">
					<div class="hidden-sm hidden-xs action-buttons">
						<a class="green" value="' . $item['id_project'] . '" href="' . base_url() . 'pmis/kontrak_engineering_edit?id_project=' . $item['id_project'] . '&id_kontrak_engineering=' . $item['id_kontrak_engineering'] . '">
							<i class="ace-icon fa fa-pencil bigger-130"></i>
						</a>
						<a class="red" value="' . $item['id_project'] . '" href="' . base_url() . 'pmis/kontrak_engineering_delete?id_project=' . $item['id_project'] . '&id_kontrak_engineering=' . $item['id_kontrak_engineering'] . '" onclick="return confirm(`Anda Yakin Menghapus Data Ini?`);">
							<i class="ace-icon fa fa-trash-o bigger-130"></i>
						</a>
					</div>
					...
					<div class="hidden-md hidden-lg">
						<div class="inline pos-rel">
							...
							<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
								<li>
									<a href="' . base_url() . 'pmis/kontrak_engineering_edit?id_project=' . $item['id_project'] . '&id_kontrak_engineering=' . $item['id_kontrak_engineering'] . '" class="tooltip-success" data-rel="tooltip" title="Edit">
										<span class="green">
											<i class="ace-icon fa fa-pencil-square-o bigger-120"></i>
										</span>
									</a>
								...
								<li>
									<a href="' . base_url() . 'pmis/kontrak_engineering_delete?id_project=' . $item['id_project'] . '&id_kontrak_engineering=' . $item['id_kontrak_engineering'] . '" onclick="return confirm(`Anda Yakin Menghapus Data Ini?`);" class="tooltip-error" data-rel="tooltip" title="Delete">
										<span class="red">
											<i class="ace-icon fa fa-trash-o bigger-120"></i>
										</span>
									</a>
	...
	public function aksi_kontrak_engineering_add()
	{
		...
		if ($this->session->userdata('status') == 2 || $this->session->userdata('status') == 3) {
			...
			//Finish Log Activity Add
			$data = array(
				'id_project' => $id_project, 'nama_kontrak_engineering' => $nama_kontrak_engineering, 'pelaksana' => $pelaksana,
	...
	public function aksi_kontrak_lokal_add()
	{
		...
		if ($this->session->userdata('status') == 2 || $this->session->userdata('status') == 3) {
			...
			$data = array(
				'id_project' => $id_project, 'nama_kontrak_lokal' => $nama_kontrak_lokal, 'pelaksana' => $pelaksana,
				'nomor' => $nomor, 'tanggal_ttd' => $tanggal_ttd,
	...
	public function aksi_progress_add()
	{
		...
		if ($this->session->userdata('status') == 2 || $this->session->userdata('status') == 3) {
			...
			$data = array(
				'id_project' => $id_project, 'tahun' => $tahun, 'bulan' => $bulan, 'individual_e' => $nilai_e, 'individual_p' => $nilai_p, 'individual_c' => $nilai_c,
	...
	public function aksi_progress_edit()
	{
		...
		if ($this->session->userdata('status') == 2 || $this->session->userdata('status') == 3) {
			...
			$data = array(
				'id_project' => $id_project, 'bulan' => $bulan, 'individual_e' => $nilai_e, 'individual_p' => $nilai_p, 'individual_c' => $nilai_c,
				'total' => $total, 'total_plan' => $total_plan,
			);
			$data_log = array('activity' => $activity,);
	...
	public function aksi_issue_add()
	{
		...
		if ($this->session->userdata('status') == 2 || $this->session->userdata('status') == 3) {
			...
			$data = array(
				'id_project' => $id_project, 'issue_description' => $issue_description, 'target_resolved_date' => $target_resolved_date, 'action' => $action,
				'status' => $status,
			);
			$data_log = array('activity' => $activity,);
	...
	public function aksi_issue_edit()
	{
		...
		if ($this->session->userdata('status') == 2 || $this->session->userdata('status') == 3) {
			...
			$data = array(
				'id_project' => $id_project, 'issue_description' => $issue_description, 'target_resolved_date' => $target_resolved_date,
	...
	public function aksi_daily_activity_add()
	{
		...
		if ($this->session->userdata('status') == 2 || $this->session->userdata('status') == 3) {
			...
			$data = array(
				'id_project' => $id_project, 'date_activity' => $date_activity, 'milstone' => $milstone,
